...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\Bloom Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\Bloom Demo Installer.exe"
    
    files = [
      projectpath + installer,
      projectpath + "\\installer\\changelog.txt",
      projectpath + "\\installer\\known-issues.txt",
      projectpath + "\\manual\\Bloom manual.pdf" 
    ]
  else:
    files = [
      projectpath + "\\build-win\\Bloom.vst3\\Contents\\x86_64-win\\Bloom.vst3",
      projectpath + "\\build-win\\Bloom_x64.exe",
      projectpath + "\\build-win\\Bloom.clap"
    ]
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\Bloom-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\Bloom-app_x64.pdb",
    projectpath + "\\build-win\\pdbs\\Bloom-clap_x64.pdb"
  ]
