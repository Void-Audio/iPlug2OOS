...
Bloom::Bloom(const InstanceInfo& info)
: iplug::Plugin(info, MakeConfig(kNumParams, kNumPresets))
{
  ...
  mLayoutFunc = [&](IGraphics* pGraphics) {
    ...
    pGraphics->AttachControl(new ITextControl(titleBounds, "Bloom", IText(30)), kCtrlTagTitle);
...
#if IPLUG_EDITOR
void Bloom::OnParentWindowResize(int width, int height)
{
  if(GetUI())
...
#if IPLUG_DSP
void Bloom::ProcessBlock(sample** inputs, sample** outputs, int nFrames)
{ 
