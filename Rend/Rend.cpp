...
Rend::Rend(const InstanceInfo& info)
: iplug::Plugin(info, MakeConfig(kNumParams, kNumPresets))
{
  ...
  mLayoutFunc = [&](IGraphics* pGraphics) {
    ...
    pGraphics->AttachControl(new ITextControl(titleBounds, "Rend", IText(30)), kCtrlTagTitle);
...
#if IPLUG_EDITOR
void Rend::OnParentWindowResize(int width, int height)
{
  if(GetUI())
...
#if IPLUG_DSP
void Rend::ProcessBlock(sample** inputs, sample** outputs, int nFrames)
{ 
