...
if %DEMO_ARG% == "demo" (
  echo Making Rend Windows DEMO VERSION distribution ...
  set DEMO=1
) else (
  echo Making Rend Windows FULL VERSION distribution ...
  set DEMO=0
)
...
REM - msbuild Rend-app.vcxproj /p:configuration=release /p:platform=win32

REM echo Building 32 bit binaries...
REM msbuild Rend.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

REM echo Building 64 bit binaries...
REM add projects with /t to build VST2 and AAX
msbuild Rend.sln /t:Rend-app;Rend-vst3;Rend-clap /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Rend.aaxplugin\Contents\Win32\Rend.aaxplugin --out .\build-win\aax\bin\Rend.aaxplugin\Contents\Win32\Rend.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Rend.aaxplugin\Contents\x64\Rend.aaxplugin --out .\build-win\aax\bin\Rend.aaxplugin\Contents\x64\Rend.aaxplugin
...
echo Making Installer ...
  ...
  REM :32-Bit-is
  REM REM "%ProgramFiles%\Inno Setup 6\iscc" /Q ".\installer\Rend.iss"
  REM goto END-is

  REM :64-Bit-is
  "%ProgramFiles(x86)%\Inno Setup 6\iscc" /Q ".\installer\Rend.iss"
  REM goto END-is

  REM :END-is

  REM - Codesign Installer for Windows 8+
  REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "Rend Installer" ".\installer\Rend Installer.exe"

  REM -if %1 == 1 (
  REM -copy ".\installer\Rend Installer.exe" ".\installer\Rend Demo Installer.exe"
  REM -del ".\installer\Rend Installer.exe"
  REM -)
